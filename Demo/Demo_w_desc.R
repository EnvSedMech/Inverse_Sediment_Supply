...
## Landsat derived SSC
SSC <- SSC_list2[[2]]
## SSC prediction location wrt the Numerical Model grid 
dgo <-dgo_list[[2]]
...
## Deposition non - uniform
## Preallocating variables
alpha <- rep(0,nc)
Cb_k  = matrix(0,k,nc)
D_k = matrix(0,k,nc)

## Entrainment non - uniform
## Preallocating variables
...
## Set initial conditions for boundary condition before iter 1
h = rep(hinitial,nc)
...
## Channel Erosion (E) and Deposition (D) in m
E <- rep(0,nc)
D <- rep(0,nc)
...
Num_model2 <- function(Css){
  ...
  while (L2_norm_h > 1e-2 | L2_norm_Q > 1e-2){
    ...
    ######################### Riverbed Gradation ##########################
    # Riverbed mixing layer thickness (m)
    delta_m = rep(1,nc)
    for(i in 2:nc-1){
...
library(mcmc)
# Simplified MCMC-based optimization using Metropolis-Hastings algorithm with adaptive scaling
...
initial_proposal_sd <- 0.000005   # Initial proposal standard deviation
scaling_factor <- 0.05   # Scaling factor for adaptive scaling

# Run MCMC-based optimization with adaptive scaling
tic()
...
par2 <- result$best_param
rmse2 <- result$best_rmse
Num_2 <- data.frame(Css = par2, RMSE = rmse2)

## Export Optimized parameter value and its associated rmse
write.csv(Num_2, file = "/N_2.csv")